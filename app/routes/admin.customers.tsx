  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold" style={{ color: '#c8102e' }}>{t('admin.customers.title')}</h1>
        <span className="text-xs" style={{ color: '#6b4a4f' }}>{t('admin.customers.total', { count: data.total.toLocaleString() })}</span>
      </div>
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex flex-wrap gap-1.5">
          {([
            { key: 'ALL',          label: t('admin.customers.filter.all'),   color: '#6b4a4f' },
            { key: 'NORMAL',       label: 'NORMAL',   color: '#15803d' },
            { key: 'PHASE_A',      label: 'PHASE_A',  color: '#b45309' },
            { key: 'PHASE_B',      label: 'PHASE_B',  color: '#c2410c' },
            { key: 'PHASE_C',      label: 'PHASE_C',  color: '#dc2626' },
            { key: 'ADMIN_LOCKED', label: 'LOCKED',   color: '#dc2626' },
        ...
        <div className="flex gap-1.5">
          {([
            ...
            { key: 'SUSPENDED', label: 'SUSPENDED', color: '#c8102e' },
      ...
      >
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {data.users.length === 0 && (
              <tr>
                <td colSpan={7} className="px-3 py-6 text-center text-xs" style={{ color: '#9c1024' }}>
            ...
            {data.users.map((u, i) => (
              <tr key={u.id} style={{ borderTop: '1px solid #f2ccd2', color: '#2b0b10' }}>
                ...
                <td className="px-3 py-2 font-semibold">
                  <a href={whatsappLink(u.tel)} target="_blank" rel="noopener noreferrer" className="hover:underline" style={{ color: '#0f766e' }} title="WhatsApp">{u.tel}</a>
                </td>
                <td className="px-3 py-2">{[u.firstName, u.lastName].filter(Boolean).join(' ') || <span style={{ color: '#8a6d71' }}>—</span>}</td>
                <td className="px-3 py-2 text-right" style={{ color: '#c8102e' }}>{u.real.toLocaleString()}</td>
                  ...
                  {!isSupport && (
                    <div className="flex items-center justify-end gap-1.5">
                      <button
                        ...
                        style={{ background: 'rgba(200,16,46,0.25)', color: '#6b4a4f', border: '1px solid #f2ccd2' }}
                      >
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            <a href={whatsappLink(u.tel)} target="_blank" rel="noopener noreferrer" className="text-sm font-semibold hover:underline" style={{ color: '#0f766e' }} title="WhatsApp">{u.tel}</a>
          </div>
      ...
      <div className="mt-2 text-xs">
        <div className="rounded-md px-2 py-1.5" style={{ background: '#ffffff' }}>
          <div className="text-[9px] font-bold " style={{ color: '#6b4a4f' }}>REAL</div>
          <div className="font-semibold" style={{ color: '#c8102e' }}>{u.real.toLocaleString()}</div>
        </div>
      </div>
...
const PHASE_LABELS: Record<SelfPlayPhase, { label: string; bg: string; color: string }> = {
  NORMAL:       { label: 'NORMAL',   bg: 'rgba(22,163,74,0.15)',   color: '#15803d' },
  PHASE_A:      { label: 'PHASE_A',  bg: 'rgba(234,88,12,0.2)',    color: '#c2410c' },
  PHASE_B:      { label: 'PHASE_B',  bg: 'rgba(202,138,4,0.2)',    color: '#a16207' },
  PHASE_C:      { label: 'PHASE_C',  bg: 'rgba(220,38,38,0.2)',    color: '#dc2626' },
  ADMIN_LOCKED: { label: 'ADMIN_LOCKED', bg: 'rgba(127,29,29,0.35)',  color: '#dc2626' },
}
...
function BetLockButton({ u, disabled, onRevalidate }: { u: CustomerRow; disabled?: boolean; onRevalidate: () => void }) {
  ...
  return (
    <button
      ...
      className="inline-flex items-center gap-1 whitespace-nowrap rounded-md px-2 py-1 text-[10px] font-bold disabled:opacity-50"
      style={{
        background: isBetLocked ? 'rgba(217,119,6,0.25)' : 'rgba(200,16,46,0.2)',
        color: isBetLocked ? '#b45309' : '#6b4a4f',
        border: `1px solid ${isBetLocked ? '#d97706' : '#f2ccd2'}`,
      }}
    >
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: '#fff5f6', border: '2px solid #f2ccd2' }}
        onClick={e => e.stopPropagation()}
      >
        <div className="mb-4 flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2 text-sm font-bold" style={{ color: '#c8102e' }}>
              <KeyRound size={14} /> {t('admin.customers.action.resetPasswordTitle')}
            </div>
            <div className="mt-0.5 text-xs" style={{ color: '#6b4a4f' }}>{name} · {user.tel}</div>
          </div>
        ...
        <fetcher.Form method="post" className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[10px] font-bold" style={{ color: '#6b4a4f' }}>{t('admin.customers.resetPassword.newPasswordLabel')}</label>
          ...
          {fetcher.data?.ok && (
            <div className="rounded-lg px-3 py-2 text-xs" style={{ background: 'rgba(22,163,74,0.15)', color: '#15803d', border: '1px solid #4ade80' }}>
