...
const TYPE_LABELS: Record<CompetitionType, { labelKey: StringKey; descKey: StringKey; color: string }> = {
  DEMO_LIVE: { labelKey: 'admin.competition.type.demoLive.label', color: '#6b4a4f', descKey: 'admin.competition.type.demoLive.desc' },
  REAL_LIVE: { labelKey: 'admin.competition.type.realLive.label', color: '#b45309', descKey: 'admin.competition.type.realLive.desc' },
  REAL_ALL:  { labelKey: 'admin.competition.type.realAll.label', color: '#c2410c', descKey: 'admin.competition.type.realAll.desc' },
}
  ...
  const ConfigForm = ({ isNew }: { isNew?: boolean }) => (
    <configFetcher.Form method="post" className="flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-1">
        <label className="text-[10px] font-bold" style={{ color: '#6b4a4f' }}>{t('admin.competition.form.typeLabel')}</label>
          ...
          {(Object.keys(TYPE_LABELS) as CompetitionType[]).map(ct => (
            <label key={ct} className="flex items-start gap-2 cursor-pointer rounded-lg px-3 py-2"
              ...
              <div>
                ...
                <div className="text-[10px]" style={{ color: '#8a6d71' }}>{t(TYPE_LABELS[ct].descKey)}</div>
              </div>
            </label>
          ))}
        </div>
      ...
      <div className="flex flex-col gap-1">
        <label className="text-[10px] font-bold" style={{ color: '#6b4a4f' }}>{t('admin.competition.form.rulesLabel')}</label>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div className="flex flex-col gap-1">
          <label className="text-[10px] font-bold" style={{ color: '#6b4a4f' }}>{t('admin.competition.form.startLabel')}</label>
        ...
        <div className="flex flex-col gap-1">
          <label className="text-[10px] font-bold" style={{ color: '#6b4a4f' }}>{t('admin.competition.form.endLabel')}</label>
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <span className="text-xs" style={{ color: '#6b4a4f' }}>{t('admin.competition.playersCount', { n: ranked.length })}</span>
      </div>
        ...
        <div className="rounded-xl p-4" style={{ background: '#fff5f6', border: '1px solid #f2ccd2' }}>
          {!showNewForm ? (
            <div className="flex flex-col items-center gap-3 py-4 text-center">
              <Trophy size={32} style={{ color: '#c8102e' }} />
              <div>
                <div className="text-sm font-bold" style={{ color: '#c8102e' }}>{t('admin.competition.blank.title')}</div>
                <div className="mt-0.5 text-xs" style={{ color: '#8a6d71' }}>{t('admin.competition.blank.subtitle')}</div>
              </div>
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center gap-2 text-sm font-bold" style={{ color: '#b45309' }}>
                <Plus size={14} /> {t('admin.competition.blank.newButton')}
              </div>
              <ConfigForm isNew />
            </div>
      ...
      {historyList.length > 0 && (
        <div className="rounded-xl overflow-hidden" style={{ background: '#fff5f6', border: '1px solid #f2ccd2' }}>
          ...
          <table className="w-full text-left text-xs hidden md:table">
            <thead>
              <tr className="text-[10px] font-bold" style={{ background: '#fff5f6', color: '#8a6d71' }}>
            ...
            <tbody>
              {historyList.map((h, i) => {
                ...
                const typeColor = h.type === 'DEMO_LIVE' ? '#6b4a4f' : '#b45309'
                ...
                return (
                  <tr key={h.id} className="cursor-pointer hover:opacity-80 transition-opacity"
                    style={{ borderTop: '1px solid #f2ccd2', color: '#2b0b10' }}
                    onClick={() => window.location.href = `/admin/competition/${h.id}`}>
                    <td className="px-3 py-2.5 text-right" style={{ color: '#8a6d71' }}>{historyList.length - i}</td>
                    <td className="px-3 py-2.5 font-semibold" style={{ color: '#c8102e' }}
                    ...
                    <td className="px-3 py-2.5 text-[10px]" style={{ color: '#9c1024' }}>
                      {h.startDate ? fmtGMT7(h.startDate) : '—'}
                    </td>
                    <td className="px-3 py-2.5 text-[10px]" style={{ color: '#9c1024' }}>
                      {fmtGMT7(h.endDate)}
                    </td>
                    <td className="px-3 py-2.5 text-right font-semibold" style={{ color: '#c8102e' }}>
            ...
            {historyList.map((h, i) => {
              ...
              return (
                <a key={h.id} href={`/admin/competition/${h.id}`}
                  ...
                  style={{ borderTop: '1px solid #f2ccd2' }}>
                  <span className="text-[10px] font-bold shrink-0" style={{ color: '#8a6d71' }}>
                    #{historyList.length - i}
                  </span>
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-xs font-semibold" style={{ color: '#c8102e' }}>{shortDetail}</div>
      ...
      {!noCompetition && (
        <div className="rounded-xl p-4"
          style={{
            background: isRunning ? 'rgba(234,179,8,0.08)' : '#fff5f6',
            border: `1px solid ${isRunning ? '#ca8a04' : '#f2ccd2'}`,
          }}>
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div className="flex items-center gap-3">
              <Trophy size={18} style={{ color: isRunning ? '#b45309' : '#9c1024' }} />
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-sm font-bold" style={{ color: isRunning ? '#b45309' : '#c8102e' }}>
                    {t(typeInfo.labelKey)}
                  </span>
                  <span className="rounded-full px-2 py-0.5 text-[10px] font-bold"
                    style={{
                      background: isRunning ? 'rgba(234,179,8,0.2)' : hasSummary ? 'rgba(74,222,128,0.15)' : 'rgba(100,116,139,0.2)',
                      color: isRunning ? '#b45309' : hasSummary ? '#15803d' : '#8a6d71',
                      border: `1px solid ${isRunning ? '#ca8a04' : hasSummary ? '#4ade80' : '#e7dcdd'}`,
                    }}>
                ...
                {competition.start && (
                  <p className="text-[10px]" style={{ color: '#6b4a4f' }}>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {isDemo && (
                <button type="button" onClick={() => setShowResetConfirm(true)}
                  ...
                  style={{ background: 'rgba(255,255,255,0.92)', color: '#6b4a4f', border: '1px solid #f2ccd2' }}>
              ...
              <summaryFetcher.Form method="post">
                ...
                <button type="submit" disabled={summaryDisabled || summaryFetcher.state !== 'idle'}
                  ...
                  className="inline-flex items-center gap-1 rounded-lg px-3 py-1.5 text-[10px] font-bold disabled:opacity-40 disabled:cursor-not-allowed"
                  style={{
                    background: (!summaryDisabled && hasSummary) ? 'rgba(22,163,74,0.2)' : 'rgba(255,255,255,0.92)',
                    color: (!summaryDisabled && hasSummary) ? '#15803d' : '#6b4a4f',
                    border: `1px solid ${(!summaryDisabled && hasSummary) ? '#16a34a' : '#f2ccd2'}`,
                  }}>
              ...
              <button type="button" onClick={() => setShowEndConfirm(true)}
                ...
                style={{
                  ...
                  color: endDisabled ? '#8a6d71' : '#dc2626',
                  border: `1px solid ${endDisabled ? '#e7dcdd' : '#ef4444'}`,
                }}>
          ...
          {hasSummary && competition.summary && (
            <div className="mt-4 border-t pt-4" style={{ borderColor: '#f2ccd2' }}>
              <div className="mb-2 text-[10px] font-bold" style={{ color: '#15803d' }}>{t('admin.competition.finalSnapshot.heading')}</div>
              <div className="flex gap-3 flex-wrap">
                {competition.summary.map(w => (
                  <div key={w.userId} className="flex items-center gap-2 rounded-lg px-3 py-2"
                    style={{ background: '#ffffff', border: `1px solid ${rankColor(w.rank)}40` }}>
                    <span style={{ color: rankColor(w.rank), fontSize: 16 }}>{rankMedal(w.rank)}</span>
                    <Avatar name={w.name ?? w.tel} src={w.profile} size={24} />
                    <div>
                      <div className="text-xs font-bold" style={{ color: '#c8102e' }}>{w.name ?? w.tel}</div>
      ...
      {!noCompetition && competition.type !== 'DEMO_LIVE' && (
        <div className="rounded-xl overflow-hidden" style={{ background: '#fff5f6', border: '1px solid #f2ccd2' }}>
          <div className="flex items-center justify-between px-4 py-3" style={{ background: '#ffffff' }}>
            <span className="text-[10px] font-bold" style={{ color: '#6b4a4f' }}>
          ...
          {participants.length === 0 ? (
            <div className="px-4 py-4 text-xs text-center" style={{ color: '#6b4a4f' }}>
              {t('admin.competition.participants.empty')}
            </div>
          ) : (
            <div className="divide-y" style={{ borderColor: '#f2ccd2' }}>
              {participants.map((p, i) => (
                <div key={p.id} className="flex items-center gap-3 px-4 py-2.5">
                  <span className="text-[10px] shrink-0 tabular-nums" style={{ color: '#8a6d71' }}>{i + 1}</span>
                  <Avatar name={p.name ?? p.tel} src={p.profile} size={28} />
                  <div className="min-w-0 flex-1">
                    <div className="text-xs font-semibold" style={{ color: '#c8102e' }}>{p.name ?? '—'}</div>
                    <div className="text-[10px]" style={{ color: '#9c1024' }}>{p.tel}</div>
                  </div>
                  <div className="text-[10px] shrink-0" style={{ color: '#8a6d71' }}>
                  ...
                  <removeFetcher.Form method="post">
                    ...
                    <button type="submit" disabled={removeFetcher.state !== 'idle'}
                      ...
                      style={{ background: 'rgba(220,38,38,0.12)', color: '#b91c1c', border: '1px solid #ef444440' }}
      ...
      {!noCompetition && (
        <div className="hidden overflow-x-auto rounded-xl md:block" style={{ background: '#fff5f6', border: '1px solid #f2ccd2' }}>
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="text-[10px] font-bold" style={{ background: '#ffffff', color: '#6b4a4f' }}>
            ...
            <tbody>
              {ranked.map(u => (
                <tr key={u.id} style={{ borderTop: '1px solid #f2ccd2', color: '#2b0b10' }}>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="text-xs font-semibold" style={{ color: '#c8102e' }}>
                          {u.name ?? <span style={{ color: '#8a6d71' }}>—</span>}
                        </div>
                        <div className="text-[10px]" style={{ color: '#8a6d71' }}>{u.id.slice(-8)}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-3 py-2.5 text-xs" style={{ color: '#6b4a4f' }}>{u.tel}</td>
                  <td className="px-3 py-2.5 text-right text-[10px]" style={{ color: '#8a6d71' }}>
                    {new Date(u.createdAt).toLocaleDateString('en-GB')}
                  </td>
                  <td className="px-3 py-2.5 text-right text-xs" style={{ color: '#dc2626' }}>{fmt(u.totalBets)}</td>
                  <td className="px-3 py-2.5 text-right">
                    <span className="font-bold" style={{ color: rankColor(u.rank) }}>{fmt(u.balance)}</span>
                    <span className="ml-1 text-[10px]" style={{ color: '#8a6d71' }}>₭</span>
                  </td>
                </tr>
              ))}
              {ranked.length === 0 && (
                <tr><td colSpan={6} className="px-3 py-8 text-center text-xs" style={{ color: '#8a6d71' }}>{t('admin.competition.ranking.empty')}</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="flex flex-col gap-2 md:hidden">
          {ranked.map(u => (
            <div key={u.id} className="flex items-center gap-3 rounded-xl p-3"
              style={{ background: '#fff5f6', border: `1px solid ${u.rank <= 3 ? rankColor(u.rank) + '40' : '#f2ccd2'}` }}>
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-sm font-bold"
                style={{ background: '#ffffff', color: rankColor(u.rank) }}>
                {u.rank <= 3 ? rankMedal(u.rank) : u.rank}
              </div>
              <Avatar name={u.name ?? u.tel} src={u.profile} size={32} />
              <div className="min-w-0 flex-1">
                <div className="truncate text-sm font-semibold" style={{ color: '#c8102e' }}>{u.name ?? '—'}</div>
              ...
              <div className="text-right">
                ...
                <div className="text-[10px]" style={{ color: '#8a6d71' }}>{t('admin.competition.ranking.betsShort', { amount: fmt(u.totalBets) })}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      {showResetConfirm && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6"
          ...
          <div className="w-full max-w-sm rounded-2xl p-6"
            style={{ background: '#fff5f6', border: '2px solid #ca8a04' }}
            onClick={e => e.stopPropagation()}>
            <h2 className="mb-1 flex items-center gap-2 text-base font-bold" style={{ color: '#b45309' }}>
            ...
            <div className="mt-5 flex gap-3">
              <button type="button" onClick={() => setShowResetConfirm(false)}
                ...
                style={{ background: '#fff0f2', color: '#9c1024', border: '1px solid #f2ccd2' }}>{t('admin.competition.form.cancel')}</button>
      ...
      {showEndConfirm && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6"
          ...
          <div className="w-full max-w-sm rounded-2xl p-6"
            style={{ background: '#fff5f6', border: '2px solid #ef4444' }}
            onClick={e => e.stopPropagation()}>
            <h2 className="mb-1 flex items-center gap-2 text-base font-bold" style={{ color: '#dc2626' }}>
